...
`htx-slab-white-on-black.png` is the approved Stacked Slab presentation supplied by the founders. The site header renders this exact artwork and uses CSS cropping plus a blend treatment so its original canvas resolves against Primary Navy; it must not be redrawn or reinterpreted.
